...
def run(options):
    ...
    with zipfile.ZipFile(str(outputPath), mode="a") as pkg:
        ...
        print(
            "Writing server into {} under {}".format(shortPath(outputPath), serverDest)
        )
    ...
    shasum = (
        subprocess.check_output(["shasum", "-a", "256", str(outputPath)])
        .decode("utf-8")
        .split()[0]
    )
...
def main(argv):
    try:
        ...
        if yarn_version is None or LooseVersion(yarn_version) < MIN_YARN_VERSION:
            eprint(
                "Yarn >=v%s is required. Current version: %s"
                % (MIN_YARN_VERSION, yarn_version)
            )
            exit(1)
